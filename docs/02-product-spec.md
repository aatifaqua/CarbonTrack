# 02 · Product Specification — CarbonTrack AI (Hybrid Offline + Gemini)

> **Canonical build spec.** Supersedes the earlier `system-requirements.md`.
> Target tier: **Advanced**. Architecture: **offline-first calculator + optional Gemini AI layer**.
> Read alongside [`03-prompt-engineering-log.md`](03-prompt-engineering-log.md) (the actual prompts).
...
| **AI (optional)** | **Gemini API** (`gemini-2.5-flash` default; `gemini-3.5-flash` if available) via `fetch` | NL logging + reduction coach |
...
### Emission factor table (every constant is cited in [`04-evaluation-and-metrics.md`](04-evaluation-and-metrics.md))
...
| Electricity · Grid | kWh | regional | kg CO2e/kWh | 17 regions — see [`04` §1.1](04-evaluation-and-metrics.md) |
...
`count / kg CO2e` rate; full provenance in [`04-evaluation-and-metrics.md`](04-evaluation-and-metrics.md)).
...
[`03-prompt-engineering-log.md`](03-prompt-engineering-log.md). This section is the *engineering* contract.
...
```
> Default model `gemini-2.5-flash` (fast, cheap, widely available). If `gemini-3.5-flash` is
> enabled on the key, prefer it. Always handle non-200 + malformed JSON by falling back.
...
4. **Validate** → run the baseline test cases in [`04-evaluation-and-metrics.md`](04-evaluation-and-metrics.md);
...
> [`03-prompt-engineering-log.md`](03-prompt-engineering-log.md) §1 (Build prompts).
...
references (provenance in [`04` §1.3](04-evaluation-and-metrics.md)):
...
Borrowed from the strongest rivals (see [`05-competitive-analysis.md`](05-competitive-analysis.md)) but
grounded in our **real per-day logs** instead of a flat counter:

- A cited **action library** (`ECO_ACTIONS`, [`04` §1.2](04-evaluation-and-metrics.md)); each carries a
...
> Why this beats rivals: streak and savings are *earned from verifiable logged days*, not vanity points,
