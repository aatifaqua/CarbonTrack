# Prompt-Engineering Log (CO-STAR)
...
```text
### CONTEXT
You are working in a repo that contains docs/product-specs.md and docs/prompt-engineering-log.md
...
```text
### CONTEXT
Building CarbonTrack AI per docs/product-specs.md. Stack: vanilla HTML5 + Tailwind(CDN v3) +
...
gemini-3.5-flash. Endpoint: POST .../v1beta/models/{model}:generateContent?key=KEY.
...
docs/evaluation-and-metrics.md and report computed vs expected: [TC-1..TC-6]. Fix anything off.
...
[`evaluation-and-metrics.md`](evaluation-and-metrics.md) §3 (e.g., injection string must yield
all-zeros + note; coach must name the true largest sector). This closes the loop between *prompt
design* (here) and *prompt validation* (there).
