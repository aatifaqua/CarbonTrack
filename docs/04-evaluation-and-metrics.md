...
> **Rigor layer (SangMin-style).** Sustainability claims must be *defensible*. This doc records the
...
> Record any change in the changelog (§5). These are *averages*; a real GHG report would use the most
...
**Pass criterion:** computed == expected to 2 dp for all six. Any drift = bug in factor wiring.
...
[`03-prompt-engineering-log.md`](03-prompt-engineering-log.md).
...
## 4. Submission scorecard (self-assessment before you submit)

Score each 1–5; target ≥ 4 everywhere. Mirrors the judging dimensions in
[`01-competition-strategy.md`](01-competition-strategy.md).

| Dimension | What "5" looks like | Self-score |
| :--- | :--- | :---: |
| Functionality | Offline calc + all 3 AI features work; degrades gracefully | ☐ |
| User experience | <1s load, equivalencies everywhere, dark theme, no dead ends | ☐ |
| Prompt-engineering skill | CO-STAR + delimiters + guardrails + JSON contracts, all in doc 03 | ☐ |
| Documentation quality | README + spec + prompt log + this doc + blog, all consistent | ☐ |
| "Wow"/memorability | Meta-sustainability story lands; demo is repeatable | ☐ |
| Validity | Live URL + repo + blog + LinkedIn linked; no secrets committed | ☐ |
...
| Date | Change |
| :--- | :--- |
| 2026-06-11 | Initial provenance table, baselines TC-1..6, prompt tests PT-1..7. |
| 2026-06-12 | Added 14 regional grid factors (§1.1) with citations; documented km/mi unit system and commute quick-add; expanded equivalency library + "closest-to-~20" selector; corrected equivalency invariants. |
| 2026-06-12 | Per-day logging: storage moved to `ct_logs` keyed by date; day navigation (arrows + calendar + Today), future dates blocked, 7/30-day trend; legacy `ct_state` auto-migrated. |
| 2026-06-12 | Added motorbike/bus + device-hour inputs (EF-5/6, EF-8/9/10); offline templated "Explain my footprint" fallback; live-computed local-DB stat; pinned Lucide; reworded payload claim to gzipped; zero-config deploy files (`vercel.json`, `.nojekyll`). |
| 2026-06-12 | Competitive upgrade: Goal & Context gauge (§1.3 averages/tiers), Eco-Actions hub with streak/cumulative/tier (§1.2), What-if Impact Simulator. All deterministic & offline; see [`05-competitive-analysis.md`](05-competitive-analysis.md). |
